...
const UserMenu = ({ onPageChange }) => {
  ...
  // Загружаем уведомления при изменении пользователя (только для не-преподавателей)
  useEffect(() => {
    if (user && !isTeacher) {
      const loadNotifications = async () => {
        try {
          ...
          // Тихая обработка ошибок - таблица notifications может не существовать
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        {/* Счетчик уведомлений (только для не-преподавателей) */}
          ...
          <>
            ...
            >
              ...
              <div className="p-4 flex-1 flex flex-col justify-start space-y-2">
                ...
                {/* Уведомления (только для не-преподавателей) */}
                {!isTeacher && (
                  <motion.button
                ...
                {isTeacher && (
                  <>
                    <motion.button
                      onClick={() => {
                        onPageChange && onPageChange("teacher-dashboard");
                        setIsOpen(false);
                      }}
                      className="w-full px-4 py-3 text-gray-700 hover:text-gray-900 hover:bg-gray-200/50 rounded-lg transition-all duration-200 flex items-center gap-3"
                      whileHover={{ x: 4 }}
