...
const LessonCreator = ({ onPageChange, pageParams = {} }) => {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      // Если передан courseId, устанавливаем его как subject_id
      setFormData(prev => ({ ...prev, subject_id: String(courseId) }));
    }
  }, [lessonId, courseId]);
  ...
  const fetchLesson = async () => {
    ...
    try {
      ...
      // Fetch materials
      const materialsRes = await apiRequest(`/lessons/${lessonId}/materials`);
      const mats = materialsRes.data?.materials ?? materialsRes.data ?? materialsRes;
      const normalized = Array.isArray(mats) ? mats.map((m) => ({
        ...m,
        material_type: (m.material_type || (m.type ? m.type.toUpperCase() : 'TEXT')),
        url: m.url ?? m.youtube_url ?? '',
        text_content: m.text_content ?? m.content ?? ''
      })) : [];
  ...
  const handleSaveLesson = async () => {
    ...
    try {
      ...
      for (const material of materials) {
        ...
        formDataObj.append('type', (material.material_type || material.type || 'text').toLowerCase());
        formDataObj.append('use_for_ai_generation', String(Boolean(material.use_for_ai_generation)));

        if (material.file) {
          formDataObj.append('file', material.file);
        } else if (material.url) {
          formDataObj.append('youtube_url', material.url);
        } else if (material.text_content) {
          formDataObj.append('content', material.text_content);
        }

        await apiRequest(`/lessons/${savedLessonId}/materials`, {
          method: 'POST',
          body: formDataObj
        });
      }
    ...
    if (materialId) {
      ...
      try {
        await apiRequest(`/materials/${materialId}`, {
          method: 'PATCH',
          body: { use_for_ai_generation: !currentValue }
        });
      } catch (err) {
