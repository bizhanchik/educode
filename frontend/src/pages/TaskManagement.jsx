...
const TaskManagement = ({ onPageChange, pageParams }) => {
  ...
  const handleGenerateAI = async (taskId) => {
    ...
    try {
      ...
      pollingRef.current.intervalId = pollInterval;
  ...
  const handleGenerateTests = async (taskId) => {
    setTestGenerationStatus((prev) => ({ ...prev, [taskId]: "loading" }));
    try {
      await generateTaskTests(taskId);
      setTestGenerationStatus((prev) => ({ ...prev, [taskId]: "completed" }));
      setSuccess("Тесты успешно сгенерированы!");
      setTimeout(() => setSuccess(""), 3000);
      loadData(); // Reload to see new stats like "Tests ready" if we added that field
    } catch (err) {
      setTestGenerationStatus((prev) => ({ ...prev, [taskId]: "error" }));
      setError(err.message || "Не удалось сгенерировать тесты");
      setTimeout(() => setError(""), 5000);
    }
  };
  ...
  const handleGradeAll = async (taskId) => {
    ...
    try {
      ...
      setSuccess(
        "Оценивание запущено. Результаты появятся через несколько минут."
      );
      setTimeout(() => setSuccess(""), 5000);
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-wrap gap-4 items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Создавайте задания, генерируйте AI решения и оценивайте работы
              студентов
            </p>
          </div>
          ...
          <div className="space-y-6">
            {tasks.map((task) => (
              <TaskCard
                ...
                testStatus={testGenerationStatus[task.id]}
    ...
    >
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700 font-medium">
              {PROGRAMMING_LANGUAGES.find((l) => l.value === task.language)
                ?.label || task.language}
            </span>
            {isExpired ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            onClick={() => onGenerateTests(task.id)}
            disabled={testStatus === "loading"}
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm hover:bg-indigo-700 transition-colors disabled:opacity-50"
          >
            {testStatus === "loading" ? (
              <>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, lesson_id: e.target.value }))
              }
              className="w-full rounded-xl border border-gray-200 px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              required
            >
