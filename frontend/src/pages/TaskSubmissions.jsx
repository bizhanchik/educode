...
const TaskSubmissions = ({ onPageChange, pageParams }) => {
  ...
  // Poll grading status while job is running
  useEffect(() => {
    if (gradingJob?.status === "running" || gradingJob?.status === "pending") {
      pollingRef.current = setInterval(async () => {
        try {
          const response = await fetchGradingStatus(taskId);
          ...
          if (jobData.status === "completed" || jobData.status === "failed") {
            ...
            if (jobData.status === "completed") {
              setSuccess("Оценивание завершено!");
              setTimeout(() => setSuccess(""), 5000);
              loadData(); // Refresh data
            } else {
              setError(
                jobData.error_message || "Оценивание завершилось с ошибкой"
              );
            }
          }
        } catch (err) {
  ...
  }, [gradingJob?.status, taskId, loadData]);

  const handleGradeAll = async () => {
    if (
      !window.confirm(
        "Запустить автоматическое оценивание всех работ? Это займет несколько минут."
      )
    ) {
      return;
    }
    ...
    setGradingJob(null);
    try {
      const response = await gradeTask(taskId);
      // Set initial job state to trigger polling
      setGradingJob({
        status: "pending",
        total_submissions:
          response.data?.submissions_to_grade || submissions.length,
        processed_submissions: 0,
        progress_percent: 0,
      });
      setSuccess("Оценивание запущено...");
    } catch (err) {
  ...
  const handleExport = async () => {
    ...
    setError("");
    try {
      await exportGrades(taskId);
      setSuccess("Файл с оценками загружен");
      setTimeout(() => setSuccess(""), 3000);
    } catch (err) {
      setError(err.message || "Не удалось экспортировать оценки");
      setTimeout(() => setError(""), 5000);
    } finally {
      setExporting(false);
    }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">
              {error || "Проверьте правильность ссылки"}
            </p>
            <button
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={exporting || evaluatedCount === 0}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-100 transition-colors disabled:opacity-50"
            >
              ...
              <div className="w-full bg-blue-200 rounded-full h-2.5">
                <div
                  ...
                  style={{
                    width: `${Math.min(
                      100,
                      gradingJob.progress_percent || 0
                    )}%`,
                  }}
                />
              </div>
              <p className="text-xs text-blue-600 mt-2">
                Прогресс: {Math.round(gradingJob.progress_percent || 0)}%
              </p>
            </div>
          )}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-700">
                  Для этого задания создано {aiSolutions.length} референсных
                  решений от AI. Используются для расчета AI Similarity
                  студенческих работ.
                </p>
                <div className="flex gap-2 mt-3">
                  {aiSolutions.map((solution, idx) => (
                    ...
                    >
                      {solution.provider === "openai"
                        ? `GPT #${solution.variant_index}`
                        : "Claude"}
                    </span>
                  ))}
                </div>
