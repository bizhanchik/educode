...
const TeacherDashboard = ({ onPageChange }) => {
  ...
  // Дополнительные состояния для навигации и редактирования
  ...
  const loadGroupStudents = useCallback(async (groupId) => {
    ...
    try {
      const response = await groupsApi.getGroup(groupId, true);
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      const studentsList = (response.data?.users || []).map((u) => ({
        id: u.id,
        name: u.name || "-",
        email: u.email || "-",
        group: u.group?.name || u.group || "-",
        status: "Активен",
      }));
  ...
  }, [currentView, loadGroups, loadStudents]);
  ...
  const handleCreateLesson = async (event) => {
    ...
    try {
      ...
      if (lessonForm.textMaterialContent.trim()) {
        ...
        textForm.append(
          "title",
          lessonForm.textMaterialTitle.trim() || "Текстовый материал"
        );
      ...
      if (lessonForm.fileMaterialFile) {
        ...
        fileForm.append(
          "title",
          lessonForm.fileMaterialTitle.trim() ||
            lessonForm.fileMaterialFile.name
        );
    ...
    return (
      <div className="max-w-4xl mx-auto mt-28 mb-12 bg-white/80 backdrop-blur rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
        ...
        <p className="text-gray-600">
          Страница доступна только для преподавателей. Пожалуйста, войдите под
          учетной записью преподавателя.
        </p>
      </div>
    );
  }
  ...
  const handleSaveLesson = () => {
    ...
    // Эта функция не используется в текущей реализации
    // Оставлена для совместимости, но не выполняет реальных действий
  ...
  const teacherNavItems = [
    ...
    { id: "create-lesson", label: "Добавить урок", icon: Plus },
      ...
      <div className="space-y-6">
        ...
        {groupsLoading ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-6 text-center">
            <p className="text-gray-600">Группы не найдены</p>
          </div>
        ) : (
        ...
        {/* Модальное окно со студентами группы */}
        {selectedGroup && (
  ...
  const renderStudents = () => {
    const filteredStudents = students.filter((student) => {
      ...
      const matchesGroup =
        !selectedStudentGroup || student.group === selectedStudentGroup;
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            Создайте свой первый курс для студентов
          </p>
          <motion.button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {subjectsWithLessons.map((course) => (
            <motion.div
              key={course.id}
            ...
            >
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600 mb-4">
                ...
                <div className="flex items-center gap-1">
                  <Users className="w-4 h-4" />0 студентов
                </div>
              </div>
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full ${
                            lesson.isCompleted ? "bg-green-500" : "bg-gray-400"
                          }`}
                        ></div>
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Алина отправила решение задачи "Сортировка массива"
              </p>
                  ...
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                      ...
                      <StatsCard
                        ...
                        value={
                          lessons.filter((lesson) => !lesson.description).length
                        }
                        accent="bg-amber-100 text-amber-600"
                      />
                      <div
                        onClick={() => onPageChange?.("task-management")}
                        className="bg-white rounded-lg shadow-md p-6 flex items-center gap-4 cursor-pointer hover:shadow-lg transition-shadow"
                      >
                    ...
                    {subjectsWithLessons.length === 0 ? (
                      <div className="rounded-3xl border border-dashed border-gray-300 bg-white p-12 text-center text-gray-500">
                        Для вас пока не создано уроков. Создайте новый урок
                        через форму выше.
                      </div>
                    ) : (
                          ...
                          >
                            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                              <div>
                                ...
                                <p className="text-gray-500 mt-1">
                                  {subject.lessons.length} урок(а/ов) в этом
                                  предмете
                                </p>
                              </div>
                              <button
                            ...
                            <div className="space-y-4">
                              {subject.lessons.map((lesson) => (
                                <div
                                  key={lesson.id}
                                  className="rounded-lg border border-gray-200 p-4 bg-gray-50 hover:bg-white transition-colors"
                                >
                                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                    <div>
                                      ...
                                      <p className="text-gray-600 mt-1">
                                        {lesson.description ||
                                          "Описание урока ещё не добавлено."}
                                      </p>
                                      {lesson.video_url && (
...
const CreateLessonCard = ({
  ...
  >
    ...
    <p className="text-gray-600 mb-6">
      Добавьте видеоурок и материалы, чтобы они стали доступны студентам на
...
const StatsCard = ({ icon: Icon, title, value, accent }) => (
  <div className="bg-white rounded-3xl border border-gray-100 p-6 flex items-center gap-4">
    <div
      className={`w-12 h-12 rounded-2xl flex items-center justify-center ${accent}`}
    >
