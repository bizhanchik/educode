import apiRequest, { fetchWithAuth } from "./apiClient.js";
...
export const createTask = async (payload) => {
  ...
  console.log("[tasksApi] Creating task:", payload);
  try {
    const response = await apiRequest("/tasks", {
      method: "POST",
      body: payload,
    });
    console.log("[tasksApi] Task created successfully:", response);
    return response;
  } catch (error) {
...
export const fetchGradingStatus = async (taskId) => {
  if (!taskId) throw new Error("taskId is required");
  return apiRequest(`/tasks/${taskId}/grading-status`);
};

export const exportGrades = async (taskId) => {
  if (!taskId) throw new Error("taskId is required");

  const response = await fetchWithAuth(`/tasks/${taskId}/export`);

  if (!response.ok) {
    throw new Error("Не удалось экспортировать оценки");
  }

  // Get filename from Content-Disposition header or use default
  const disposition = response.headers.get("Content-Disposition");
  let filename = `grades_task_${taskId}.csv`;
...
export const fetchSubmissions = async ({
  ...
  if (hasEvaluation !== undefined)
    params.append("has_evaluation", String(hasEvaluation));
...
export const fetchAllAISolutions = async ({
  page = 1,
  size = 50,
  taskId,
  provider,
...
export const generateAISolutions = async (taskId) => {
  if (!taskId) throw new Error("taskId is required");
  return apiRequest(`/ai-solutions/task/${taskId}/generate`, {
    method: "POST",
  });
};
...
export const generateTaskTests = async (taskId) => {
  if (!taskId) throw new Error("taskId is required");
  return apiRequest(`/tasks/${taskId}/generate-tests`, {
    method: "POST",
  });
};

export const fetchTaskTests = async (taskId) => {
  if (!taskId) throw new Error("taskId is required");
  return apiRequest(`/tasks/${taskId}/tests`);
};
