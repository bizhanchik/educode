...
async def create_lesson_assignment(
    ...
    try:
        ...
        if current_user.role == UserRole.TEACHER and lesson.teacher_id != current_user.id:
            raise HTTPException(status_code=403, detail="Access denied: You can only assign your own lessons")
...
async def get_lesson_assignments(
    ...
    try:
        conditions = []
        # Teachers see only their lessons; students see only their group's assignments; admins see all
        if current_user.role == UserRole.TEACHER:
            conditions.append(Lesson.teacher_id == current_user.id)
        elif current_user.role == UserRole.STUDENT and current_user.group_id:
            conditions.append(LessonAssignment.group_id == current_user.group_id)
        if lesson_id:
...
async def get_active_assignments_for_group(
    ...
    try:
        # Students can only view their own group's assignments
        if current_user.role == UserRole.STUDENT and current_user.group_id != group_id:
            raise HTTPException(status_code=403, detail="Access denied: You can only view your group's assignments")
...
async def update_lesson_assignment(
    ...
    try:
        ...
        if current_user.role == UserRole.TEACHER:
            lesson_result = await db.execute(
                select(Lesson).where(Lesson.id == assignment.lesson_id)
            )
            lesson = lesson_result.scalar_one_or_none()
            if lesson and lesson.teacher_id != current_user.id:
                raise HTTPException(status_code=403, detail="Access denied: You can only update assignments for your lessons")
...
async def delete_lesson_assignment(
    ...
    try:
        ...
        if current_user.role == UserRole.TEACHER:
            ...
            lesson = lesson_result.scalar_one_or_none()
            if lesson and lesson.teacher_id != current_user.id:
                raise HTTPException(status_code=403, detail="Access denied: You can only delete assignments for your lessons")
